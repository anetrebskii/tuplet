...
export const headCommand: CommandHandler = {
  ...
  help: {
    ...
    flags: [
      ...
      { flag: '-c NUM', description: 'Output first NUM characters' }
    ],
    examples: [
  ...
  async execute(args: string[], ctx: CommandContext): Promise<ShellResult> {
    ...
    // Character mode helper
    function sliceByChars(content: string): string {
    ...
    if (paths.length === 0 && ctx.stdin !== undefined) {
      const output = chars !== null ? sliceByChars(ctx.stdin) : sliceByLines(ctx.stdin)
    ...
    for (const path of paths) {
      ...
      const output = chars !== null ? sliceByChars(content) : sliceByLines(content)
